...
void Client::send_reconnect(MetaSession *session)
{
  ...
  // re-send flushing caps which were revoked
  kick_revoked_flushing_caps(session);
...
void Client::flush_caps(Inode *in, MetaSession *session, bool late_kick)
{
  ...
  for (int i = 0; i < CEPH_CAP_BITS; ++i) {
    ...
    if (late_kick && late_kick_flush_tids.count(tid) == 0)
      continue;
    flushes[tid] |= 1 << i;
  }
...
void Client::kick_flushing_caps(MetaSession *session)
{
  ...
  late_kick_flush_tids.clear();
}

void Client::kick_revoked_flushing_caps(MetaSession *session)
{
  late_kick_flush_tids.clear();
  ...
  for (xlist<Inode*>::iterator p = session->flushing_caps.begin(); !p.end(); ++p) {
	...
	late_kick_flush_tids.insert(in->flushing_cap_tid[i]);
    }
